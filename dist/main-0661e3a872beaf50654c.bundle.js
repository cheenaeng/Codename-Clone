...
"use strict";
module.exports = JSON.parse('{"_from":"axios@^0.21.1","_id":"axios@0.21.4","_inBundle":false,"_integrity":"sha512-ut5vewkiu8jjGBdqpM44XxjuCjq9LAKeHVmoVfHVzy8eHgxxq8SbAVQNovDA8mVi05kP0Ea/n/UzcSHcTJQfNg==","_location":"/axios","_phantomChildren":{},"_requested":{"type":"range","registry":true,"raw":"axios@^0.21.1","name":"axios","escapedName":"axios","rawSpec":"^0.21.1","saveSpec":null,"fetchSpec":"^0.21.1"},"_requiredBy":["/"],"_resolved":"https://registry.npmjs.org/axios/-/axios-0.21.4.tgz","_shasum":"c67b90dc0568e5c1cf2b0b858c43ba28e2eda575","_spec":"axios@^0.21.1","_where":"/Users/cheenaeng/Desktop/rocket-academy/bootcamp/Projects/Project3-CODENAMES","author":{"name":"Matt Zabriskie"},"browser":{"./lib/adapters/http.js":"./lib/adapters/xhr.js"},"bugs":{"url":"https://github.com/axios/axios/issues"},"bundleDependencies":false,"bundlesize":[{"path":"./dist/axios.min.js","threshold":"5kB"}],"dependencies":{"follow-redirects":"^1.14.0"},"deprecated":false,"description":"Promise based HTTP client for the browser and node.js","devDependencies":{"coveralls":"^3.0.0","es6-promise":"^4.2.4","grunt":"^1.3.0","grunt-banner":"^0.6.0","grunt-cli":"^1.2.0","grunt-contrib-clean":"^1.1.0","grunt-contrib-watch":"^1.0.0","grunt-eslint":"^23.0.0","grunt-karma":"^4.0.0","grunt-mocha-test":"^0.13.3","grunt-ts":"^6.0.0-beta.19","grunt-webpack":"^4.0.2","istanbul-instrumenter-loader":"^1.0.0","jasmine-core":"^2.4.1","karma":"^6.3.2","karma-chrome-launcher":"^3.1.0","karma-firefox-launcher":"^2.1.0","karma-jasmine":"^1.1.1","karma-jasmine-ajax":"^0.1.13","karma-safari-launcher":"^1.0.0","karma-sauce-launcher":"^4.3.6","karma-sinon":"^1.0.5","karma-sourcemap-loader":"^0.3.8","karma-webpack":"^4.0.2","load-grunt-tasks":"^3.5.2","minimist":"^1.2.0","mocha":"^8.2.1","sinon":"^4.5.0","terser-webpack-plugin":"^4.2.3","typescript":"^4.0.5","url-search-params":"^0.10.0","webpack":"^4.44.2","webpack-dev-server":"^3.11.0"},"homepage":"https://axios-http.com","jsdelivr":"dist/axios.min.js","keywords":["xhr","http","ajax","promise","node"],"license":"MIT","main":"index.js","name":"axios","repository":{"type":"git","url":"git+https://github.com/axios/axios.git"},"scripts":{"build":"NODE_ENV=production grunt build","coveralls":"cat coverage/lcov.info | ./node_modules/coveralls/bin/coveralls.js","examples":"node ./examples/server.js","fix":"eslint --fix lib/**/*.js","postversion":"git push && git push --tags","preversion":"npm test","start":"node ./sandbox/server.js","test":"grunt test","version":"npm run build && grunt version && git add -A dist && git add CHANGELOG.md bower.json package.json"},"typings":"./index.d.ts","unpkg":"dist/axios.min.js","version":"0.21.4"}');
...
var socket = socket_io_client__WEBPACK_IMPORTED_MODULE_1__.io.connect();
/* -------------------------------------------------------------------------- */
...
var assignTeamClass = function assignTeamClass(command, element, startingColor, secondColor) {
  element.firstElementChild.innerHTML = "";

  switch (command) {
    case 'startingTeam':
      element.classList.add("".concat(startingColor));
      break;

    case 'secondTeam':
      element.classList.add("".concat(secondColor));
      break;
...
var currentTeamTurnDiv = document.querySelector('.show-current-team');
var roomBtn = document.querySelector('.room-btn');
var submitRoomCodeBtn = document.querySelector('#submit-room-number');
var submitGameUserDetailsBtn = document.querySelector('#submit-gameuser-btn');
var guessSubmitBtn = document.querySelector('#guesses-submit');
var submitHostBtn = document.querySelector('#submit-host');
var usernameBtn = document.querySelector('#choose-username-btn');
var joinRoomBtn = document.querySelector('.join-room-btn');
var spyColor; //to manipulate

/* --------------------- LOBBY AREA ELEMENTS AND LOG IN --------------------- */

var playersFieldEl = document.querySelector('.players-input');
var hostUserNameEl = document.querySelector('.host-user');
var submitPlayersBtn = document.querySelector('#submit-players-number');
var lobbyDiv = document.querySelector('.lobby-wrapper');
var mainHeader = document.querySelector('.game-load');
var enterRoomCodeEl = document.querySelector('.room-code-input');
var guestUsernameWrapper = document.querySelector('.choose-username-wrapper');
/* ----------------------------- MAIN GAMEBOARD ----------------------------- */

var mainGameBoard = document.querySelector('.main-gamedashboard');
var gameBoardContainer = document.querySelector('.dashboard-container');
/* -------------------------------------------------------------------------- */
...
var submitPlayers = function submitPlayers(e) {
  e.target.classList.add('hide');
  console.log(e.target);
  joinRoomBtn.classList.add('hide');
  playersFieldEl.classList.remove('hide');

  var createRoomCode = function createRoomCode() {
    var playersInput = document.querySelector('#player-numbers').value;
    var playerData = {
      playerNumbers: playersInput
    };
    axios__WEBPACK_IMPORTED_MODULE_2___default().post('/rooms', playerData).then(function (response) {
      hostUserNameEl.classList.remove('hide');
      playersFieldEl.classList.add('hide');
      var roomID = response.data.createRoom.id;
      var roomCodeDiv = document.querySelector('#show-room-code');
      roomCodeDiv.innerHTML = "".concat(response.data.createRoom.roomCode);
      axios__WEBPACK_IMPORTED_MODULE_2___default().post('/createGame', roomID).then(function (res) {
...
var showLobbyArea = function showLobbyArea(room) {
  console.log("lobby area shown");
  var chooseRoleDiv = document.querySelector('.choose-role'); // chooseRoleDiv.classList.add('hide')

  var submitGameUserBtn = document.querySelector('#submit-gameuser-btn');
  var redTeamMessageDiv = document.querySelector('.red-team-message');
  var blueTeamMessageDiv = document.querySelector('.blue-team-message');
  var teamRadioEl = document.querySelectorAll('input[name="team"]');
  socket.emit('findTeam');
  socket.on('returnTeam', function (data) {
    console.log('yes');
    console.log(data);
    var currentRedTeam = data.teamChosen.teamPlayerCount[0];
    var currentBlueTeam = data.teamChosen.teamPlayerCount[1];
    var totalRedTeam = Math.floor(data.playersNumber / 2);
    var totalBlueTeam = data.playersNumber - totalRedTeam;
    redTeamMessageDiv.innerHTML = "".concat(currentRedTeam, " out of ").concat(totalRedTeam, " have joined red team");
    blueTeamMessageDiv.innerHTML = "".concat(currentBlueTeam, " out of ").concat(totalBlueTeam, " have joined blue team");
    var spyMessageDiv = document.querySelector('.spy-msg');
    var nonSpyMessageDiv = document.querySelector('.non-spy-msg');
    teamRadioEl.forEach(function (radio) {
      return radio.addEventListener('input', function () {
        socket.emit('findTeam');
        socket.on('returnTeam', function (data) {
          ...
          var checkTeamSelected = (_document$querySelect = document.querySelector('input[name="team"]:checked')) === null || _document$querySelect === void 0 ? void 0 : _document$querySelect.value;
          var isRedSpyAval = data.teamChosen.redSpy;
          var isBlueSpyAval = data.teamChosen.blueSpy;
          console.log(isBlueSpyAval);
          console.log(checkTeamSelected); //if player chooses red or blue and spy role already taken 
          ...
          if (isBlueSpyAval && checkTeamSelected === "blue") {
            ...
            if (!isRedSpyAval && checkTeamSelected === "red" && currentRedTeam === totalRedTeam - 1 || !isBlueSpyAval && checkTeamSelected === "blue" && currentBlueTeam === totalBlueTeam - 1) {
...
var createGameUser = function createGameUser(userData) {
  axios__WEBPACK_IMPORTED_MODULE_2___default().post('/gameuser', userData).then(function (response) {
    var gameuserDetail = {
      username: userData.username,
      roomId: response.data.createGameUser.roomId
    };
    var gameUserId = response.data.createGameUser.id;
    console.log(response.data, "gameusers data");
    socket.emit('join room', gameuserDetail);
    socket.emit('join');
    socket.on('hello message', function () {
      console.log('hellllooooooo');
      var messageDiv = document.querySelector('.message-users-joined');
      ...
      console.log(args);
      messageDiv.innerHTML = "".concat(args[0], " have joined the room");
      showLobbyArea(gameuserDetail.roomId);
    });
    socket.on('full room', function () {
      var fullRoomMsg = document.querySelector('.full-room-message');
...
var enterUsername = function enterUsername() {
  var usernameInput = document.querySelector('#choose-username').value;
  var userInfo = {
...
var submitHost = function submitHost() {
  var hostName = document.querySelector('#username-host').value;
  var dataUser = {
    host: hostName
  };
  axios__WEBPACK_IMPORTED_MODULE_2___default().post('/createHost', dataUser).then(function (response) {
...
var guestJoinRoom = function guestJoinRoom(e) {
  enterRoomCodeEl.classList.remove('hide');
  e.target.classList.add('hide');
  roomBtn.classList.add('hide');
};

var checkRoomcode = function checkRoomcode() {
  var roomCodeInput = document.querySelector('#room-code-input').value;
  var roomInput = {
    roomCode: roomCodeInput
  };
  axios__WEBPACK_IMPORTED_MODULE_2___default().post('/joinroom', roomInput).then(function (response) {
...
var checkForCurrentTeam = function checkForCurrentTeam(colorSpy) {
  console.log(colorSpy);
  var mainGameBoard = document.querySelectorAll('#universal-game-board .col');
  socket.on('returnGameStatus', function (data) {
    console.log('return current game status');
    currentTeamTurnDiv.innerHTML = "CURRENT TEAM IS: ".concat(data.gamestate.currentTeam);

    if (data.gamestate.currentTeam != colorSpy) {
      document.querySelector('#guesses-input').disabled = true;
      mainGameBoard.forEach(function (element) {
...
var receiveChangeInCards = function receiveChangeInCards() {
  socket.on('reveal', function (data) {
    console.log(data, 'data');
    socket.emit('findGameStatus');
    var cardElDiv = document.querySelector(".card-backface-".concat(data.cardValue));
    var cardElDivParent = document.querySelector(".card-backface-".concat(data.cardValue)).parentNode;
...
var flipCard = function flipCard(gameDetails) {
  socket.emit('showCards', gameDetails);
}; //data here stores the game information 


var checkUseridentity = function checkUseridentity(data) {
  //to show general information to all 
  mainGameBoard.classList.remove('hide'); //if starting team is red, to show red start first, else otherwise 

  var ifRedStarts = document.querySelector('.if-starting-team-red');
  var guessOutputRed = document.querySelector('.required-guess-red');
  var ifBlueStarts = document.querySelector('.if-starting-team-blue');
  var guessOutputBlue = document.querySelector('.required-guess-blue');
  ...
  axios__WEBPACK_IMPORTED_MODULE_2___default().get('/gameuserIdentity').then(function (response) {
    var showTeamDiv = document.querySelector('.show-team');
    var teamOfUser;
    console.log(response.data);
    response.data.findGameuser.isRed ? teamOfUser = 'red' : teamOfUser = 'blue';
    showTeamDiv.innerHTML = "Team: ".concat(teamOfUser);
    var mainBoard = document.querySelector('#universal-game-board .gameboard-container');
    mainBoard.classList.remove('hide');
    var showRole = document.querySelector('.show-role');

    if (!response.data.findGameuser.isSpy) {
      showRole.innerHTML = 'ROLE: FIELD OPERATIVE';
      var allCardsElement = document.querySelectorAll('#universal-game-board .card-btn');
      ...
      var spyBoard = document.querySelector('.small-revealed-board .gameboard-container');
      spyBoard.classList.remove('hide');
      console.log(spyBoard); //to assign cards to each board 

      var cardsShuffled = data.cards.allWords;
      console.log(cardsShuffled);
      var boardBox = document.querySelectorAll('.card-btn-main');
      var spyBoardEl = document.querySelectorAll('.small-revealed-board .card-btn');
      console.log(spyBoardEl);
      console.log(boardBox);

      for (var i = 0; i < boardBox.length; i += 1) {
        boardBox[i].setAttribute('id', "".concat(i));
        boardBox[i].classList.add("card-backface-".concat(i));
        spyBoardEl[i].innerHTML = cardsShuffled[i];
        boardBox[i].innerHTML = cardsShuffled[i];
      } //small board for spy master to see the cards 


      var _mainGameBoard = document.querySelectorAll('#universal-game-board .col .card-btn-main');

      _mainGameBoard.forEach(function (card) {
        console.log(card);
        card.addEventListener('click', function (e) {
          console.log(e.target);
          var getCardValue = e.target.id;
          var cardValueCheck = {
          ...
          var gameData = {
            ...
            secondTeamColor: secondTeam,
            cardValueCheck: cardValueCheck,
            roomId: data.roomId
          }; //spy will trigger this event

          flipCard(gameData); //everytime click the div to check whether spycolor is same as current team 
      ...
      var mappingWord = [];
      spyBoardEl.forEach(function (card, index) {
        var checkWord = cardsShuffled[index];

        for (var category in data.cards) {
          if (data.cards[category].includes(checkWord)) {
            mappingWord[index] = category;
          }
        }
      });
      var startingTeamColor = data.gamestate.startingTeam;
      var secondColorTeam;
      startingTeamColor === 'blue' ? secondColorTeam = 'red' : secondColorTeam = 'blue'; //to assign colors to the spyboard 

      for (var _i = 0; _i < spyBoardEl.length; _i += 1) {
        var spyBoardElparent = document.querySelectorAll('.small-revealed-board .card-btn')[_i].parentNode;

        spyBoardEl[_i].innerHTML = "";
        assignTeamClass(mappingWord[_i], spyBoardElparent, startingTeamColor, secondColorTeam);
      }
...
var getGuessValue = function getGuessValue() {
  var guessValue = document.querySelector('#guesses-input').value;
  console.log(guessValue); //need  to know who is the current team

  socket.emit('newGame');
  socket.on('returnGame', function (gameResult) {
    console.log('return');
...
var showGuessValue = function showGuessValue() {
  socket.on('showGuess', function () {
    var guessMessageDiv = document.querySelector('.show-guess');
    guessMessageDiv.innerHTML = "Number of guess: ".concat(arguments.length <= 0 ? undefined : arguments[0]);
  });
};
/* -------------------------------------------------------------------------- */
...
var initGame = function initGame() {
  var lobbyContainerEl = document.querySelector('.lobby-wrapper');
  lobbyContainerEl.classList.add('hide');
  socket.emit('newGame');
  socket.on('returnGame', function (gameData) {
  ...
  showGuessValue();
  socket.emit('findGameStatus');
  socket.on('returnGameStatus', function (result) {
    console.log(result);
    currentTeamTurnDiv.innerHTML = "".concat(result.gamestate.currentTeam);
  });
}; //this is to update game user details when player press the submit button 


var submitGameDetails = function submitGameDetails() {
  var checkTeamSelected = document.querySelector('input[name="team"]:checked').value;
  var roleSelected = document.querySelector('input[name="role"]:checked').value; //to update isSpy and isRed column under games_users table 

  var userDetailsSelected = {
    role: roleSelected,
  ...
  axios__WEBPACK_IMPORTED_MODULE_2___default().put("/gameuserPlay", userDetailsSelected).then(function (response) {
    console.log(response.data);
  });
  axios__WEBPACK_IMPORTED_MODULE_2___default().put('/roomTeamDetails', userDetailsSelected).then(function (response) {
    console.log(response.data); // showLobbyArea(response.data.roomData.roomId)
